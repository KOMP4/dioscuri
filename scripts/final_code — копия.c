...
void bmp_get_data()
{
	...
	if (!((buf & 0b00001000) >> 3))
	{
		...
		raw_press = ((signed long int)bmp_raw_data[0]) << 12 | ((signed long int)bmp_raw_data[1]) << 4 | bmp_raw_data[0] >> 4;
...
uint8_t DS_find()  //поиск датчика на шине 1-WIRE
{
  ...
  */
   ...
   if(DS_find())
   {
    ...
    {
     DS_send(0b00111111);
     ds_convert_delay = 190;
     break;
    }
    case 2:
    {
     DS_send(0b01011111);
     ds_convert_delay = 380;
     break;
    }
    default:
     DS_send(0b01111111);
     ds_convert_delay = 760;
    }
   }
   HAL_TIM_Base_Stop_IT(&htim3);
...
//-----------------------------LIS------------------------------------------
   ...
   void lis_get_data()
   {
     ...
     printf("Mag X = %f, Mag Y = %f, Mag Z = %f\n", lis_data_float[0], lis_data_float[1], lis_data_float[2]);
 ...
 //----------------------------LSM-------------------------------------------
   ...
   void lsm_get_data()
   {
 	  ...
 	  if(buf > 0)
 	  {
 	      ...
 	      printf("Gyro X = %f, Gyro Y = %f, Gyro Z = %f\n",data_lsm_float[0],data_lsm_float[1],data_lsm_float[2]);
 	      printf("Axel X = %f, Axel Y = %f, Axel Z = %f\n",data_lsm_float[3],data_lsm_float[4],data_lsm_float[5]);
...
void nrf_lsm() //данные с lsm
{
	for (int i = 14; i < 20; ++i)
	{
		data_nrf[i] = data_lsm_float[i - 14];
	}
}
...
void nrf_check_sum() //функция для расчёта контрольной суммы
{
	...
	for (int i = 1; i < 20; ++i) //цикл для выполнения логической операции "исключающее ИЛИ" из всех байт информации
	{
...
void nrf_transmit()  //функция для подготовки данных к отправке и сама отправка данных
{
  ...
  if (NRF_msg_type)  // Проверяем на тип сообщения, если тип сообщения 1, то отправляем в конце пакета магнитометр
  {
    ...
    data_nrf[23] = lis_raw_data[0];
    data_nrf[24] = lis_raw_data[1];
    data_nrf[25] = lis_raw_data[2];
    data_nrf[26] = lis_raw_data[3];
    data_nrf[27] = lis_raw_data[4];
    data_nrf[28] = lis_raw_data[5];
  ...
  else  // Если тип сообщения 0, то отправляем гироскоп
  {
    ...
    data_nrf[23] = data_lsm[0]; 
    data_nrf[24] = data_lsm[1];
    data_nrf[25] = data_lsm[2];
    data_nrf[26] = data_lsm[3];
    data_nrf[27] = data_lsm[4];
    data_nrf[28] = data_lsm[5];
